...
class FirebaseManager:
    ...
    def update_counts(self, camera_id, camera_name, counts):
        if self.db:
            try:
                # Update traffic_police collection with live data for CBS Dashboard
                doc_ref = self.db.collection("traffic_police").document(camera_id)
                inc = counts.get("incoming", {})
                out = counts.get("outgoing", {})
                total = sum(inc.values()) + sum(out.values())

                status = 'Congested' if total > 100 else ('Clear' if total < 50 else 'Moderate')
                
                data = {
                    "junctionId": camera_id,
                    "junctionName": camera_name,
                    "timestamp": firestore.SERVER_TIMESTAMP,
                    "timestamp_local": datetime.now().isoformat(),
                    "liveVehicleCount": total,
                ...
                # Keep history for analytics (traffic_data)
                # Here we only record every so often by calling this infrequently if needed
                self.db.collection("traffic_data").add({
                    "camera_id": camera_id,
...
class CameraProcessor:
    def __init__(self, cam_cfg: Dict, output_dir: str, firebase_mgr=None):
        ...
        self.events = []  
        self.target_names = ['car', 'bus', 'truck', 'motorcycle']
        self.class_map = {}
        self.writer = None
    ...
    def process(self, max_frames: int = None, verbose: bool = True):
        ...
        try:
            for result in stream:
                ...
                if self.firebase_mgr and (current_time - last_firebase_update) > 2.0:
                    self.firebase_mgr.update_counts(self.id, self.name, self.counts)
...
def run_proc(cam_cfg, output_dir, firebase_mgr, max_frames):
    proc = CameraProcessor(cam_cfg, output_dir=output_dir, firebase_mgr=firebase_mgr)
    proc.process(max_frames=max_frames, verbose=True)
...
def main(config_path, output_dir, firebase_cert=None, max_frames: int = None):
    ...
    # Let's run all 4 cameras simultaneously using threads
    for cam in cameras:
        t = threading.Thread(target=run_proc, args=(cam, output_dir, firebase_mgr, max_frames), daemon=True)
        t.start()
        threads.append(t)
        time.sleep(1)
        
    print(f"Started {len(threads)} cameras simultaneously in background threads...")
    try:
        while True:
            time.sleep(10)
    except KeyboardInterrupt:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--firebase-cert', default="firebase-adminsdk.json", help='Path to firebase-adminsdk.json')
